...
export default class HelloWorldScene extends Phaser.Scene {
  ...
  create() {
    ...
    //Cielo
    this.add.image(420, 278, "Aldea").setScale(0.57);
   ...
   const objects = this.physics.add.group();
    ...
    const creaForma = () => {
      ...
      const obj = objects.create(x, y, type).setScale(0.5); // Crear objeto
          ...
          if (type === "diamante") {
            ...
            obj.setTint(0x00ff00); // Cambiar color a verde
           obj.Puntos = 5;
           obj.Tiempo = 1;
      } 
          ...
          if (type === "triangulo") {
          // obj.setVelocity(Phaser.Math.Between(-100, 100)); // Velocidad aleatoria
           obj.setTint(0x0000ff); // Cambiar color a azul
           obj.Puntos = 10;
           obj.Tiempo = 0.5;
      }
      ...
      obj.setVelocityY(Phaser.Math.Between(50, 100));
      // Rebote al chocar con la plataforma
      obj.setBounce(0.5);
    ...
    //--- Temporizador
    this.time.addEvent({
      delay: 500, // Intervalo de 5 segundos
    ...
    this.physics.add.overlap(ninja, objects, (ninja, obj) => {
      this.puntos += obj.Puntos; // Sumar puntos al ninja
  ...
  //   // update game objects
    ...
    if (Phaser.Input.Keyboard.JustDown(this.cursors.Barra)) {
      if (this.cursors.A.isDown) {
        // Aumentar la velocidad hacia la izquierda
        this.ninja.setVelocity(
          this.ninja.body.velocity.x - 30000, // Reducir velocidad en X
          this.ninja.body.velocity.y + 50  // Ajustar velocidad en Y
        );
      }
      if (this.cursors.D.isDown) {
        // Aumentar la velocidad hacia la derecha
        this.ninja.setVelocity(
          this.ninja.body.velocity.x + 30000, // Aumentar velocidad en X
          this.ninja.body.velocity.y + 50  // Ajustar velocidad en Y
        );
      }
    }
